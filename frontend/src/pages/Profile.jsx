...
function Profile() {
  ...
  const handleSave = async () => {
    if (!editData.fullName || !editData.university) { toast.error('Please fill in all fields'); return; }
    setSaving(true);
    try {
  ...
  const badges = [
    ...
    { icon: '🏆', name: 'Top Scorer', desc: 'Scored 90%+ average', earned: stats.avgScore >= 90 },
      ...
      <div className="max-w-3xl mx-auto p-8 space-y-6">
            ...
            {editing ? (
              <div className="space-y-3">
                <div>
                  ...
                  <input type="text" value={editData.fullName} onChange={(e) => setEditData({ ...editData, fullName: e.target.value })} className="w-full px-4 py-2 border-2 border-gray-200 rounded-xl focus:outline-none focus:border-secondary text-gray-700 text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-500 mb-1">University</label>
                  <input type="text" value={editData.university} onChange={(e) => setEditData({ ...editData, university: e.target.value })} className="w-full px-4 py-2 border-2 border-gray-200 rounded-xl focus:outline-none focus:border-secondary text-gray-700 text-sm" />
                </div>
              ...
              <>
                ...
                <div className="space-y-1 mt-2">
                  <p className="flex items-center gap-2 text-gray-500 text-sm"><FaEnvelope className="text-primary" /> {user?.email}</p>
                  <p className="flex items-center gap-2 text-gray-500 text-sm"><FaUniversity className="text-primary" /> {user?.university}</p>
                </div>
                <div className="mt-3">
                  <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold ${user?.plan === 'pro' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-600'}`}>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button onClick={() => setLanguage('en')} className={`p-4 rounded-xl border-2 font-medium transition ${language === 'en' ? 'border-primary bg-blue-50 text-primary' : 'border-gray-200 text-gray-600 hover:border-gray-300'}`}>
              🇬🇧 English
            </button>
            <button onClick={() => setLanguage('fr')} className={`p-4 rounded-xl border-2 font-medium transition ${language === 'fr' ? 'border-primary bg-blue-50 text-primary' : 'border-gray-200 text-gray-600 hover:border-gray-300'}`}>
              🇫🇷 Français
            </button>
          </div>
          {language !== (user?.language || 'en') && (
            <button onClick={handleSave} className="mt-3 w-full bg-primary text-white py-2 rounded-xl text-sm font-medium hover:bg-secondary transition">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-3 gap-3">
            {badges.map((badge, i) => (
              <div key={i} className={`p-4 rounded-xl text-center border-2 transition ${badge.earned ? 'border-yellow-300 bg-yellow-50' : 'border-gray-100 bg-gray-50 opacity-50'}`}>
                <div className="text-3xl mb-2">{badge.icon}</div>
                <p className="font-bold text-gray-800 text-xs">{badge.name}</p>
                <p className="text-gray-500 text-xs mt-0.5">{badge.desc}</p>
                {badge.earned && <span className="inline-block mt-1 text-xs text-yellow-600 font-bold">✅ Earned</span>}
              </div>
            ))}
          </div>
        </div>
        ...
        {user?.plan !== 'pro' && (
          <div className="bg-gradient-to-r from-primary to-secondary rounded-2xl p-6 text-white shadow-lg">
            ...
            <p className="text-blue-100 text-sm mb-4">Get unlimited notes, AI summaries, and 50+ quiz questions per note.</p>
            <div className="grid grid-cols-2 gap-3">
              <button className="bg-white text-primary font-bold py-3 rounded-xl hover:bg-blue-50 transition text-sm">📱 Pay with MTN MoMo</button>
              <button className="bg-white text-primary font-bold py-3 rounded-xl hover:bg-blue-50 transition text-sm">🟠 Pay with Orange Money</button>
            </div>
          </div>
        )}

        <button onClick={handleLogout} className="w-full flex items-center justify-center gap-3 bg-white border-2 border-red-200 text-red-500 font-bold py-4 rounded-2xl hover:bg-red-50 transition">
